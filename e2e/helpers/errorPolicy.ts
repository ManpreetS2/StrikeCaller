 ...
 *
 * Same-origin StrikeCaller URLs are never ignorable, even when the Chromium
 * token is ERR_NO_BUFFER_SPACE or ERR_INSUFFICIENT_RESOURCES.
 *
...
export function isIgnorableConsoleError(input: ConsoleErrorInput): boolean {
  ...
  if (urls.some(isStrikeCallerOwnedUrl) || looksLikeStrikeCallerAssetInText(input.text)) {
    return false
  }
